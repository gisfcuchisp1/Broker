...
Ext.onReady(function () {
    ...
    var location_style_context = function (feature) {
        if (!feature.geometry) {
            // expects a value to lookup on initial load before geometry exists
            return {geom: 'other'};
        }
    ...
    var historyCombo = new Ext.form.ComboBox({
        ...
        displayField: 'name',
        valueField: 'period',
        // default is entries that are still current
        value: 0,
    ...
    function renderOpButton(val) {
        if (val.ATOM) {
            return '<button type="button" style="color: blue;" onclick="MASAS.load_past_entry(\'' +
                val.ATOM.href + '\')">' + MASAS.UPDATE_OPERATION + '</button>';
        }
    }
    ...
    var selectEntryCard = new Ext.FormPanel({
        ...
        items: [ {xtype: 'compositefield',
            ...
            items: [ historyCombo, {
                ...
                checked: true,
                allowBlank: true,
                handler: MASAS.load_user_entries
            }, {
                boxLabel: 'Other Allowed',
                xtype: 'checkbox',
                style: 'margin-left: 25px;',
        ...
        }, selectMapPanel, MASAS.selectGridPanel ]
    });
    ...
    var intervalCombo = new Ext.form.ComboBox({
        ...
        store: new Ext.data.ArrayStore({
            ...
            data: [ [null, '', 'Do not set expires'],
                ...
                // past and is acting as an Entry version of Cancel
    ...
    var optionalEntryCard = new Ext.FormPanel({
        ...
        items: [{
            ...
            id: 'entry-add-permission-update',
            items: [{
                xtype: 'radiogroup',
    ...
    var postEntryCard = new Ext.Panel({
        ...
        items: [{
            ...
            html: '<div class="postingCard" id="postingBox"><h1>Ready to Post</h1>' + 
                '<a href="#" onclick="MASAS.preview_entry_xml(); return false;" ' +
                'style="color: grey;">Preview</a><a href="#" onclick="MASAS.' + 
                (('Update' === MASAS.UPDATE_OPERATION) ? 'post_entry_update' : 'post_new_entry') +
    ...
    var entryPanel = new Ext.Panel({
        ...
        title: MASAS.UPDATE_OPERATION + ' Entry',
        region: 'center',
        layout: 'card',
        activeItem: 0,
        bodyStyle: 'padding: 10px;',
        ...
        items: [ selectEntryCard, inputEntryCard, locationEntryCard, optionalEntryCard,
            postEntryCard ]
    });
...
//TODO: purge attachments here too?
...
MASAS.load_user_entries = function () {
    ...
    // either look for editable by this author or created by this author
    if (allowed_val) {
        feed_url += '&update_allow=' + MASAS.USER_URI;
    } else {
        feed_url += '&author=' + MASAS.USER_URI;
    }
    // default is 0 for entries that are still current
    if (history_val !== 0) {
...
MASAS.load_past_entry = function (atom_link) {
    if (!atom_link) {
    ...
    var value_result = MASAS.load_past_values(xml_doc);
    if (!value_result) {
...
MASAS.load_past_values = function (xml_doc) {
    if (!MASAS.oldAtom) {
    ...
    Ext.getCmp('entry-add-permission-update').collapse();
    ...
    MASAS.attachmentStore.removeAll();
    MASAS.attachmentStore.commitChanges();
    
    for (var i = 0; i < MASAS.oldAtom.entry.category.length; i++) {
        var cat_attrib = MASAS.oldAtom.entry.category[i];
        if (cat_attrib['@scheme'] === 'masas:category:status') {
            Ext.getCmp('entry-status').setValue(cat_attrib['@term']);
            MASAS.oldEntryStatus = cat_attrib['@term'];
            ...
            if (node_match) {
                ...
                var icon_url = MASAS.ICON_PREVIEW_URL + cat_attrib['@term'] +
                    '/large.png';
                Ext.getCmp('icon-preview-box').setValue('<div style="padding: 15px;"><img src="' +
                    icon_url + '"></div>');
            } else {
    ...
    if (MASAS.oldAtom.entry.title.div.div instanceof Array) {
        // multiple divs for each language
        for (var j = 0; j < MASAS.oldAtom.entry.title.div.div.length; j++) {
            if (MASAS.oldAtom.entry.title.div.div[j]['@xml:lang'] === 'en') {
                found_en_title = true;
                Ext.getCmp('entry-en-title').setValue(MASAS.xml_decode(MASAS.oldAtom.entry.title.div.div[j]['#text']));
            } else if (MASAS.oldAtom.entry.title.div.div[j]['@xml:lang'] === 'fr') {
                found_fr_title = true;
                Ext.getCmp('entry-fr-title').setValue(MASAS.xml_decode(MASAS.oldAtom.entry.title.div.div[j]['#text']));
            }
        }
    } else if (typeof(MASAS.oldAtom.entry.title.div.div) === "object") {
        if (MASAS.oldAtom.entry.title.div.div['@xml:lang'] === 'en') {
            found_en_title = true;
            Ext.getCmp('entry-en-title').setValue(MASAS.xml_decode(MASAS.oldAtom.entry.title.div.div['#text']));
        }
    }
    ...
    if (MASAS.oldAtom.entry.content['@type'] === 'xhtml') {
        if (MASAS.oldAtom.entry.content.div.div instanceof Array) {
            // multiple divs for each language
            for (var k = 0; k < MASAS.oldAtom.entry.content.div.div.length; k++) {
                if (MASAS.oldAtom.entry.content.div.div[k]['@xml:lang'] === 'en') {
                    Ext.getCmp('entry-en-content').setValue(MASAS.xml_decode(MASAS.oldAtom.entry.content.div.div[k]['#text']));
                } else if (MASAS.oldAtom.entry.content.div.div[k]['@xml:lang'] === 'fr') {
                    Ext.getCmp('entry-fr-content').setValue(MASAS.xml_decode(MASAS.oldAtom.entry.content.div.div[k]['#text']));
                }
            }
        } else if (typeof(MASAS.oldAtom.entry.content.div.div) === "object") {
            if (MASAS.oldAtom.entry.content.div.div['@xml:lang'] === 'en') {
                Ext.getCmp('entry-en-content').setValue(MASAS.xml_decode(MASAS.oldAtom.entry.content.div.div['#text']));
            }
        }
    }
    
    // there should always be multiple links to check, 
    if (MASAS.oldAtom.entry.link instanceof Array) {
        for (var l = 0; l < MASAS.oldAtom.entry.link.length; l++) {
            if (MASAS.oldAtom.entry.link[l]['@rel'] === 'related') {
                // if there are multiple related links, using the last to update for now
                Ext.getCmp('entry-en-related').setValue(MASAS.oldAtom.entry.link[l]['@href']);
            }
            if (MASAS.oldAtom.entry.link[l]['@rel'] === 'enclosure' &&
            MASAS.oldAtom.entry.link[l]['@type'] !== 'application/common-alerting-protocol+xml') {
                var import_data = {
                    type: MASAS.oldAtom.entry.link[l]['@type'],
                    url: OpenLayers.Util.urlAppend(MASAS.oldAtom.entry.link[l]['@href'],
                        'secret=' + MASAS.USER_SECRET)
                };
                if ('@title' in MASAS.oldAtom.entry.link[l]) {
                    import_data['title'] = MASAS.oldAtom.entry.link[l]['@title'];
                } else {
                    import_data['title'] = 'Attachment';
                }
                var do_import = new OpenLayers.Request.POST({
                        url: MASAS.IMPORT_ATTACH_URL,
                ...
                if (do_import.status === 201 || do_import.status === 200) {
                    var new_rec = new MASAS.attachmentStore.recordType({
                            ...
                            num: parseInt(do_import.responseText, 10)
                        });
                    ...
                    console.error('Error importing attachment: ' +
                        MASAS.oldAtom.entry.link[l]['@href']);
    ...
    if (MASAS.oldAtom.entry['app:control']) {
        if (MASAS.oldAtom.entry['app:control']['mec:update']) {
            if (MASAS.oldAtom.entry['app:control']['mec:update'].search(/all/) !== -1) {
                Ext.getCmp('entry-update-control').setValue('all');
                Ext.getCmp('entry-add-permission-update').expand();
            }
        }
    }
    ...
    if (georss_geometries.length > 0) {
        ...
        if (MASAS.oldAtom.entry['georss:circle']) {
            ...
            MASAS.locationMap.getLayersByName('Location')[0].addFeatures([center_feature]);
            location_feature = circle_feature;
        }
        if (MASAS.oldAtom.entry['georss:box']) {
            location_feature.is_box = true;
        }
        MASAS.save_location_feature(location_feature, true);
        ...
        if (MASAS.oldAtom.entry['georss:point']) {
            ...
            map_panel.center = point_latlon;
            map_panel.zoom = 12;
            MASAS.locationMap.setCenter(point_latlon, 12);
        } else {
            ...
            map_panel.extent = extent_bounds;
            MASAS.locationMap.zoomToExtent(extent_bounds);
        }        
    } else {
...
MASAS.save_location_feature = function (feature, no_popup) {
...
MASAS.generate_entry_xml = function () {
    ...
    var new_categories = [];
    new_entry.entry.category[0]['@term'] = MASAS.oldEntryStatus;
...
MASAS.preview_entry_xml = function () {
    ...
    preview_xml_vals[1].entry.id = 'Entry ' + MASAS.UPDATE_OPERATION;
    try {
...
MASAS.post_entry_update = function () {
    var posting_html = '<h1>Posting Entry</h1><div class="postingWait"></div>';
    document.getElementById('postingBox').innerHTML = posting_html;
    MASAS.postingTimer = setTimeout(MASAS.post_timeout_error, 15000);
    // once a post is made, don't allow going back to retry with this session's data
    ...
    if (MASAS.oldAtom.entry.link instanceof Array) {
        for (var i = 0; i < MASAS.oldAtom.entry.link.length; i++) {
            if (MASAS.oldAtom.entry.link[i]['@rel'] === 'edit') {
                edit_url = MASAS.oldAtom.entry.link[i]['@href'];
            }
            if (MASAS.oldAtom.entry.link[i]['@rel'] === 'edit-media') {
                media_url = MASAS.oldAtom.entry.link[i]['@href'];
            }
        }
    } else if (typeof(MASAS.oldAtom.entry.link) === "object") {
        if (MASAS.oldAtom.entry.link['@rel'] === 'edit') {
            edit_url = MASAS.oldAtom.entry.link['@href'];
        }
    }
    if (edit_url === null) {
    ...
    if (MASAS.USE_ATTACH_PROXY === true && media_url !== null) {
        console.debug('Posting attachment updates first');
        var put_media = new OpenLayers.Request.PUT({
                url: MASAS.ATTACH_PROXY_URL,
                data: OpenLayers.Util.getParameterString({
                    url: OpenLayers.Util.urlAppend(media_url, 'secret=' + MASAS.USER_SECRET)
                }),
                headers: {'Content-Type': 'application/x-www-form-urlencoded'},
                async: false
            });
